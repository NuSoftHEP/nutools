...
// nutools/SimulationBase included
#ifndef ART_V1
...
// for now this is local to dunendtf  (hopefully will move to nutools)
#include "GENIE2ART.h"
...
namespace evg {
  ...
  struct AddGenieEventsToArtParams {
    ...
    Table<VtxOffsets> vtxOffsets {
      Name("vtxOffsets"),
      Comment("allow module to offset global vertex")
    };
    Atom<bool>        addMCFlux {
      Name("addMCFlux"),
  ...
  while ( entries.size() != n ) {
    if ( ! fRandomEntries ) {
      ...
      if ( std::find(entries.begin(),entries.end(),indx) != entries.end() ) {
        // mf::LogInfo("AddGeniEventsToArt") << "rejecting " << indx << " as already there";
      } else {
